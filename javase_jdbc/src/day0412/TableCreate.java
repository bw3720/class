...
public class TableCreate extends JFrame implements ActionListener {
	...
	private void setTableName(String tName) {
		if(createQuery.toString().contains("create")) {
			JOptionPane.showMessageDialog(this, "create 쿼리가 이미 생성되었습니다. 삭제 후 다시 만들어 주세요.");
			return;
		}//end if
	...
	private void setColumn(String columnName, String dataType, String columnSize, boolean pk, boolean nn) {
		...
		if(pk) {
			column.append(" constraint pk_").
			append(createQuery.substring(createQuery.indexOf("table")+6,createQuery.indexOf("("))).append(" primary key");
		}//end if
	   ...
	   public void actionPerformed(ActionEvent ae) {
	      if(ae.getSource() == jbtnTableName) {
	         ...
	         if("".equals(jtfTableName.getText().trim())) {
	            JOptionPane.showMessageDialog(this, "테이블명은 필 수 입력입니다.");
	            return;
	         }//end if
	      ...
	      if(ae.getSource() == jbtnReset) {
	         switch(JOptionPane.showConfirmDialog(this, "DDL쿼리문을 삭제 하시겠습니까?")) {
	      ...
	      if(ae.getSource() == jbtnSetCol) {
	         ...
	         if("".equals(jtfColName.getText().trim())) {
	            JOptionPane.showMessageDialog(this, "컬럼명은 필수 입력입니다.");
	            return;
	         }//end if
	         ...
	         if(jcbPmKey.isSelected() && createQuery.toString().contains("primary key")) {
	        		JOptionPane.showMessageDialog(this, "테이블에 primary key가 이미 존재합니다.");
	        		return;
	        	}//end if
	         //N.N
	         if(jcbNN.isSelected() && jcbPmKey.isSelected()) {
	        	 jcbNN.setSelected(false);//Primary key 인 상태에서는 N.N을 해제한다.
	         }//end if
	         
	         setColumn(jtfColName.getText().replace(" ", ""), dataType, jtfSize.getText().replace(" ", ""), jcbPmKey.isSelected(), jcbNN.isSelected());
	      }//end if
	      ...
	      if(ae.getSource()== jbtnCreate) {
	    	  ...
	    	  try {
				...
				JOptionPane.showMessageDialog(this, "테이블이 이미 존재 합니다.");
				e.printStackTrace();
			}//end catch
	      }//end if
