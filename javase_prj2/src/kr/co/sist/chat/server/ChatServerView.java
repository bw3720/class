...
public class ChatServerView extends JFrame implements ActionListener, Runnable {
	...
	public static List<ChatServerHelper> connList;// 접속자를 관리하기위한 리스트
	private ServerSocket server;//접속자 소켓을 받기위한 ServerSocket
	private Thread connThread;//접속자를 디자인과 별도로 받기위해 동시에 처리하기위한 Thread
...
//		connList= new ArrayList<ChatServerHelper>();  //동시접근 가능
		connList= new Vector<ChatServerHelper>();  //동시접근 불가능
	...
	public void run() {
		try {
			...
			JOptionPane.showMessageDialog(this,	"서버 구동중 문제가 발생하였습니다.");
			e.printStackTrace();
		}//end catch
	}//run
	...
	public void actionPerformed(ActionEvent ae) {
		if(connThread ==null) {//서버가 실행된적 없을 때 => 서버소켓을 생성하여 접속자를 받는다.
			connThread= new Thread(this);
			connThread.start(); // ==> run();실행
