...
if(ldVO==null){
	...
	//////////쿠키에 아이디를 설정.//////////
	//1. 아이디저장 체크박스 체크되었는지 얻기.
	String idSaveFlag=request.getParameter("idFlag");
	
	if(idSaveFlag!=null){//value속성이 없는 상태에서 check하면 "on"이 들어온다.
		//2. 쿠키생성. (파라메터로 받은 아이디를 save_id라는 이름으로 쿠키 저장)
		Cookie idCookie=new Cookie("save_id",id);
		//3. 쿠키 생존시간(파기시간)을 설정.(접속자가 브라우저를 종료한 후 시간)
		idCookie.setMaxAge(60);
		//4. 쿠키 심기.(HttpServletResponse을 이용)
		response.addCookie(idCookie);
	}else{//checkbox가 체크되지 않은 상태. 쿠키를 삭제해야하는 상태.
 	 	//쿠키를 삭제.
		//1.삭제하려는 이름의 쿠키를 생성
