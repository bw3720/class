		...
		$.ajax({
			...
			success: function( jsonObject ){
				if( jsonObject.resultFlag ){
					var output="<table class='table-bordered'><tr><td style='width:80px'>이미지</td><td style='width:120px'>차량명</td>"
					+"<td style='width:300px'>옵션</td>"
					+"<td style='width:60px'>년식</td>"
					+"<td style='width:80px'>가격</td>"
					+"<td style='width:80px'>배기량</td></tr>";
					...
					$.each(jsonObject.data,function(i,jsonCar){
						...
						+jsonCar.car_year+"</td><td>"+jsonCar.price+"</td><td>"+jsonCar.cc+"</td></tr>";
					});
					
					$("#model").html(output);
				}else{
					$("#model").html("<stong>서버에서 문제가 발생하였습니다.</strong>");
				}
			}
		});//ajax
