...
public class DispatherServlet extends HttpServlet {
	...
	@Override
	public void init() {//최초 접속자가 페이지를 요청했을 때 한번만 호출되는 method.
		//Map을 설정
		map.put("M001",new MemberFormAction());//회원가입 양식.
		map.put("D001",new IdDupAction());//중복확인.
		map.put("M002",new MemberProcessAction());//회원가입처리.
		
	}//init
	
	
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//Get방식으로 요청하면 Post방식으로 처리.
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(cmd == null) {//실행되는 Action을 선택하도록 만드는 외부입력 값.
			cmd="M001";//cmd가 없다면(최초호출 or 외부에서 cmd를 훼손) 회원가입 form을 보여준다.
		}//end if
		
		Action action = map.get(cmd);//입력된 cmd값에 해당하는 action의 자식클래스를 map에서 찾아 꺼내온다.
		
		if(action == null) {//cmd값을 외부에서 훼손하여 다른 값으로 입력했을 때
			action=map.get("M001");//메인화면(회원가입)으로 제공한다.
		}//end if
		
		//꺼내온 action의 method를 호출.(Override된 자식클래스의 method가 호출)
		
		action.execute(request, response);//파라메터 처리, 유효성 검증, 관계유지객체사용, service객체의 사용.
